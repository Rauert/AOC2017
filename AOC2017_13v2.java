...
//Part 2
		int i = 0;
		while (traverse(i) == false) {
			i++;
		}
		System.out.println("Part 2: Delay = " + (i));
		
	}

	private static boolean traverse(int offset) {
		//boolean rtn = false;
		//int pos = 0 - offset;
		//int sum = 0;
		//int currRange;
...
//			}
			for (int i = 0; i < data.size(); i++) {
				int time = i + offset;
				if (data.get(i) > 1 && (time % ((data.get(i)-1) * 2) == 0)) {
					return false;
